...
applicationDescriptor.sources.each { sourceGroup ->
	...
	sourceGroup.files.each { file ->
		...
		if (types != null && types.size() > 0 && !types[0].equals("UNKNOWN")) {
			types.each() { type ->
				ArrayList<String> filesMapped = typesToFilesMap.get(type)
				if (!filesMapped) {
					filesMapped = new ArrayList<String>()
					typesToFilesMap.put(type, filesMapped)
				}
				filesMapped.add("${repositoryPath}/${file.name}.${fileExtension}")
			}
		}
	}
}

if (typesToFilesMap && typesToFilesMap.size() > 0) {
	...
	typesToFilesMap.each() { type, fileList ->
		...
		if (typeConfiguration) {
			...
			File yamlFile = new File("${zBuilderConfigurationFolderPath}/${typeConfiguration.typeConfiguration}.yaml")
			...
			ArrayList<IncludeFile> includeFiles = applicationDBBAppYaml.application.include
			if (!includeFiles) {
				includeFiles = new ArrayList<IncludeFile>()
				applicationDBBAppYaml.application.include = includeFiles
			}
			IncludeFile newIncludeFile = new IncludeFile()
			newIncludeFile.file = "zBuilder-config/${typeConfiguration.typeConfiguration}.yaml"
			includeFiles.add(newIncludeFile)
		} else {
...
matchingSourcesGroup.each() { matchingMourceGroup ->
	...
	if (impactAnalysisTask) {
		println("found impactAnalysisTask")
