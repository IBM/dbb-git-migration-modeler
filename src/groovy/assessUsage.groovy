        ...
        if (!targetApplicationMappingFile.exists()) {
            ...
            try {
                ...
                while((line = sourceApplicationMappingReader.readLine()) != null) {
                    ...
                    if ("${lineSegments[1]}".equals("${props.DBB_MODELER_APPLICATION_DIR}/${sourceApplication}/${oldFileLocation}")) {
                        lineSegments[1] = "${props.DBB_MODELER_APPLICATION_DIR}/${targetApplication}/${targetRepositoryPath}"
                ...
                if (foundMatch) {
                    logger.logMessage("\t==> Moved mapping definition for file ${sourceApplication}/${oldFileLocation} from ${configurationsDirectory}/${sourceApplication}.mapping to ${targetApplication}/${targetRepositoryPath} in ${configurationsDirectory}/${targetApplication}.mapping.")
                } else {
                    logger.logMessage("\t==> [WARNING] Update of DBB Migration Mapping files for ${sourceApplication}/${oldFileLocation} failed. ${sourceApplication}/${oldFileLocation} was not found in ${configurationsDirectory}/${sourceApplication}.mapping. The DBB Migration Mapping files ${configurationsDirectory}/${sourceApplication}.mapping and ${configurationsDirectory}/${targetApplication}.mapping were not updated.")
                }
            }
            catch (IOException e) {
                e.printStackTrace()
