...
logger.logMessage("** Reading the Types Mapping definition.")
if (props.APPLICATION_MEMBER_TYPE_MAPPING) {
    def typesMappingFile = new File(props.APPLICATION_MEMBER_TYPE_MAPPING)
    if (!typesMappingFile.exists()) {
        ...
        def yamlSlurper = new groovy.yaml.YamlSlurper()
        typesMapping = yamlSlurper.parse(typesMappingFile)
    }
} else {
...
/* def isFilterOnMemberMatching(String memberName, String filter) {
	println("INSIDE match - $memberName - $filter")
    return memberName.matches(filter)
} */
...
def generateApplicationFiles(String application) {
    ...
    datasetMembersCollection.each () { datasetMember ->
        ...
        def memberType = getTypes(member)
...
def getTypes(String member) {
    if (!typesMapping) {
        return "UNKNOWN"
    } else {
    	ArrayList<String> matchingTypes = new ArrayList<String>()
        typesMapping.types.each() { type ->
        	type.filePatterns.each() { filePattern ->
	            if (isFilterOnMemberMatching(member, filePattern)) {
	            	matchingTypes.add(type.type)
	            }
        	}
        }
        if (matchingTypes) {
            return matchingTypes.toString().replaceAll("[\\[\\] ]", "")
        } else {
            return "UNKNOWN"
        }
    }
