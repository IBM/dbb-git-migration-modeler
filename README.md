...
The file controls how dataset members should be dispatched to target subfolders on USS, depending on the low-level qualifiers of the dataset which hold them or their associated types (if any, as described in the [Types file](./types.txt)).  
...
`./1-extractApplications.sh -d DBEHM.MIG.COBOL,DBEHM.MIG.COPY --applicationMapping $DBB_MODELER_HOME/applicationMappings.yaml --repositoryPathsMapping $DBB_MODELER_HOME/repositoryPathsMapping.yaml --types $DBB_MODELER_HOME/types.txt -o $DBB_MODELER_APPCONFIGS`
