...
| DBB_MODELER_DEFAULT_GIT_CONFIG  | The configuration folder containing default `.gitattributes` and `zapp.yaml` files. | The default value is computed based on the DBB Git Migration Modeler working folder, where a `config/git-config` subfolder is created: `$DBB_MODELER_WORK/config/git-config` |
...
| DBB_MODELER_APPMAPPINGS_DIR  | The configuration folder containing the Applications Mapping file(s) defining the input datasets for applications and their naming conventions. More information can be found in [Configuring the Migration Modeler input files](03-Configuration.md#configuring-the-migration-modeler-input-files). | The default value is computed based on the DBB Git Migration Modeler working folder, where a `config/application-mappings` subfolder is created: `$DBB_MODELER_WORK/config/application-mappings` | 
